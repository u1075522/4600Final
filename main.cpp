...
void computeJointTransformations(
	// There are total ​p_numJoints transformations in ​p_local and ​p_offset 

	// local joint transformations, if the character is animating, it will be changing everytime 
	// when this function is called
	// "T" in lecture
	const std::vector<Matrix4f>& p_local,	
	// relative transformations between individual joints, not changing even it is animating
	// "R" in lecture
	const std::vector<Matrix4f>& p_offset, 
	// Storing the index of parent joint of joint [j] 
	...
	std::vector<Matrix4f>& p_global)
{
	...
	//		for (int k = 0; k < p_numJoints; k++) {
	//			p_global[i][j][k] = p_global[p_jointParent[i]][j][k];
	//		}
	//	}
	//}
	...
	// Set the value in position 0 first
	p_global[0] = p_offset[0] * p_local[0];
	// using the algorithm that provided in slides
	for (int i = 1; i < p_numJoints; i++) {
...
void skinning(
		...
		// rest pose vertex positions, convert it to homogeneous coordinates
		const std::vector<Vector3f>& p_vertices, 
		// number of joints 
		const unsigned int p_numJoints,
		// p_global
		const std::vector<Matrix4f>& p_jointTrans,
		// These are concatenated joint transformations computed at the rest pose and inverted
		const std::vector<Matrix4f>& p_jointTransRestInv,
		// p_weights[j][v] contains the skinning weight for vertex v and j
		...
		std::vector<Vector3f>& p_deformedVertices)
{
	...
	for (unsigned int v = 0; v < p_vertices.size(); v++)
	{
		...
		Vector4f homo = toHomog(p_vertices[v]);
		Vector4f result;
		...
		// following formula to compute value
		for (int k = 0; k < p_numJoints; k++) {
			result = result + p_weights[k][v] * p_jointTrans[k] * p_jointTransRestInv[k] * homo;
		}
		// homo conversion
		p_deformedVertices[v] = fromHomog(result);
